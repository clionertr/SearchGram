...
13. `/add_sync <chat_id>`: Add a chat ID to the sync list
14. `/remove_sync <chat_id>`: Remove a chat ID from the sync list
15. `/list_sync`: List all chat IDs in the sync list

Other Commands:
16. `/ping`: Check bot and database status
...
@app.on_message(filters.command(["start_client", "stop_client", "restart_client"]))
@private_use
def manage_client(client: Client, message: types.Message):
    command = message.text.split()[0][1:]  # 获取命令名称
    client_script = "/opt/sg/client.py"
    log_file = "/opt/sg/client.log"
...
def manage_sync(client: Client, message: types.Message):
    ...
    config.read("/opt/sg/sync.ini")

    if command == "add_sync":
        if len(args) != 1:
            message.reply_text("Usage: /add_sync <chat_id>")
            return
        chat_id = args[0]
        if "sync" not in config:
            config["sync"] = {}
        config["sync"][chat_id] = "true"  # 使用字符串 "true" 而不是 None
        with open("/opt/sg/sync.ini", "w") as configfile:
            config.write(configfile)
        message.reply_text(f"Added {chat_id} to sync list.")

    elif command == "remove_sync":
        if len(args) != 1:
            message.reply_text("Usage: /remove_sync <chat_id>")
            return
        chat_id = args[0]
        if "sync" in config and chat_id in config["sync"]:
            del config["sync"][chat_id]
            with open("/opt/sg/sync.ini", "w") as configfile:
                config.write(configfile)
            message.reply_text(f"Removed {chat_id} from sync list.")
        else:
            message.reply_text(f"{chat_id} not found in sync list.")

    elif command == "list_sync":
        if "sync" in config and config["sync"]:
            sync_list = "\n".join(config["sync"].keys())
            message.reply_text(f"Current sync list:\n{sync_list}")
        else:
            message.reply_text("Sync list is empty.")
